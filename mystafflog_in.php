    ...
    if (empty($reservationId) || empty($parkingLotId)) {
        ...
        if ($validateResult->num_rows > 0) {
            ...
            if ($reservationStatus === 'Booked') {
                // Reservation status is 'Booked', proceed with updating the reservation status to 'Active'
                // Query to update the reservation status to 'Active'
                $updateSql = "UPDATE reservation_tb SET reservation_status = 'Active' WHERE reservation_id = '$reservationId' AND parking_lot_id = '$parkingLotId'";
